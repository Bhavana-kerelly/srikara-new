...
const POSTERS = [
  {
    ...
    hoverDuration: 4000     // 4.0s when hovered
  },
  {
    id: 'hrudaya',
  ...
  {
    id: 'op200',
    title: 'Miyapur OP Consultation ₹200',
  ...
  return (
    <AnimatePresence>
      ...
      >
        ...
        {/* ── BOTTOM SPACING ── */}
        <footer className="relative z-10 w-full max-w-6xl mx-auto h-4 flex items-center justify-center">
          <div className="flex items-center gap-2">
            {POSTERS.map((p, idx) => (
              <span
                key={p.id}
                className={`transition-all duration-300 rounded-full ${
                  currentIndex === idx ? 'w-6 h-2 bg-[#8B1A4A]' : 'w-2 h-2 bg-slate-300'
                }`}
              />
            ))}
          </div>
