...
function DoctorCard({ doctor, isActive, transitionEnabled, onClick, getHasDragged, isMobile }) {
  ...
  const handleCall = (e) => {
    e.stopPropagation();
    window.location.href = `tel:${doctor.phone || '9247958308'}`;
  };

  const handleWhatsapp = (e) => {
    e.stopPropagation();
    const message = encodeURIComponent(`Hi Dr. ${doctor.name}, I would like to book an appointment.`);
    window.open(`https://wa.me/${doctor.whatsapp || '919247958308'}?text=${message}`, '_blank');
  };
  ...
  return (
    ...
    >
      ...
      <motion.div 
        ...
        transition={currentCardTransition}
        className="relative w-full overflow-hidden flex items-end justify-center flex-shrink-0 pt-3"
      >
        ...
        <img
          src={doctor.image}
          alt={doctor.name}
          className="relative z-10 w-auto max-w-[92%] h-[85%] max-h-[85%] object-contain object-bottom transition-transform duration-700"
          style={{
...
export const PremiumDoctorFinder = ({ branchTitle = 'ECIL', branchId = 'ECIL', isGlobal = false, allowBranchSwitch = false }) => {
  ...
  // Filter doctors for this specific branch and ensure Dr. Akhil Dadi is always first
  const branchDoctors = useMemo(() => {
    const normSel = normalize(selectedBranch);
    let list = doctors;
    if (normSel && normSel !== 'hospitals' && normSel !== 'all') {
      list = doctors.filter(doc => normalize(doc.branch) === normSel);
    } else {
      list = doctors.filter((doc, index, self) => index === self.findIndex(t => (t.slug || t.id) === (doc.slug || doc.id)));
    }
  ...
  // Set the first card (Dr. Akhil Dadi) active initially
  useEffect(() => {
    setSlideIndex(N);
  }, [N, selectedBranch]);
    ...
    <section className="py-24 bg-[#FAF5F7] relative overflow-hidden font-sans">
      ...
      {(isGlobal || allowBranchSwitch) && (
        <div className="max-w-[1400px] mx-auto px-8 relative z-10 mb-12 flex flex-wrap justify-center md:justify-start gap-3">
          {['ECIL', 'Madeenaguda', 'Miyapur', 'Peerzadiguda', 'L.B. Nagar', 'Kompally', 'Lakdikapul', 'Rajahmundry', 'RTC X Roads', 'Vijayawada'].map((branchName) => {
      ...
      <div className="max-w-[1400px] mx-auto px-8 mt-10 flex justify-center relative z-10">
        <button
          onClick={() => navigate(selectedBranch ? `/doctors?branch=${encodeURIComponent(selectedBranch)}` : '/doctors')}
