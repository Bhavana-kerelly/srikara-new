...
export function DoctorsGroupBanner({ branchName }) {
  ...
  const akhilDadi = useMemo(() => {
    return doctors.find(d => d.slug === 'akhil-dadi' || d.slug === 'dr-akhil-dadi') || doctors[0];
  }, [doctors]);

  // Full team of this branch (for the expanded drawer)
              ...
              >
                ...
                <div className="flex-1 text-[#2D3A4A] relative z-10 max-w-xl">
                  ...
                  <h3 className="font-garamond text-5xl sm:text-6xl font-bold leading-tight mb-6 text-[#2D3A4A]">
                    {akhilDadi?.name || 'Dr. Akhil Dadi'}
                  </h3>

                  <p className="text-slate-600 text-base md:text-lg font-light leading-relaxed mb-8">
                    {akhilDadi?.about || akhilDadi?.bio || 'A visionary in robotic arthroplasty, Dr. Akhil Dadi brings world-class German fellowship expertise to Srikara Hospitals, leading the team with over 29+ years of clinical mastery.'}
                  </p>
                  ...
                  <div className="flex gap-4">
                    <button
                      onClick={() => navigate(`/doctors/${akhilDadi?.slug || 'dr-akhil-dadi'}`)}
                    ...
                    <button
                      onClick={() => navigate(`/book/${akhilDadi?.slug || 'dr-akhil-dadi'}`)}
                ...
                <div className="relative w-full lg:w-[420px] aspect-[4/5] overflow-hidden rounded-[2rem] border border-[#8B1A4A]/10 bg-gradient-to-tr from-[#8B1A4A]/10 to-[#8B1A4A]/5 flex items-end justify-center self-end">
                  ...
                  <img
                    src={akhilDadi?.image}
                    alt={akhilDadi?.name || 'Dr. Akhil Dadi'}
                    className="w-auto h-[90%] object-contain object-bottom relative z-0 pointer-events-none"
                    onError={e => { if (akhilDadi?.fallback) e.target.src = akhilDadi.fallback; }}
                  />
                </div>
                    ...
                    >
                      ...
                      <div className="relative aspect-[4/4.5] overflow-hidden bg-slate-50 flex items-end justify-center">
                        ...
                        <img 
                          ...
                          alt={doctor.name}
                          style={{
                            transform: (doctor.imageScale && doctor.imageScale !== 1.0) ? `scale(${doctor.imageScale})` : undefined,
