...
const getGreetingMessage = (pathname, currentBranch) => {
  ...
  if (cleanPath.startsWith('/specialties')) {
    return "Hi! Welcome to our Specialties section. Looking for a specialized department? 🌟"
  }
...
const getAIResponse = (userQuery) => {
  ...
  if (query.includes('neuro') || query.includes('brain') || query.includes('spine') || query.includes('stroke') || query.includes('tumor')) {
    return {
      text: "Srikara's Stroke and Neuro-Endovascular clinic delivers microscopic, ultra-rapid interventions for brain tumors, spinal corrections, and neurological anomalies. Would you like to check our neurology specialists?",
      action: "view_doctors",
      actionLabel: "Find Neurologists",
      link: "/doctors?specialty=neuro"
    }
  }
...
export function AppointmentWidget({ currentBranch }) {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      alert(`Appointment request submitted successfully for ${formData.name}! Opening WhatsApp to confirm with our team...`)
      
      const waMsg = encodeURIComponent(
        `Hello Srikara Hospitals,\n\nI have submitted an appointment request via the website widget:\n\n` +
        `👤 *Patient Name*: ${formData.name.trim()}\n` +
        `📱 *Mobile*: ${formData.phone.trim()}\n` +
        `🏥 *Branch*: ${branchName}\n` +
        `⚕️ *Specialty*: ${formData.specialty || 'General Consultation'}\n\n` +
        `Please contact me to confirm the slot. Thank you!`
      )
      window.open(`https://wa.me/919247958308?text=${waMsg}`, '_blank')
