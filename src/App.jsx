...
import { lbNagar, kompally, lakdikapul, ecil, miyapur, madeenaguda, vijayawada, rajahmundry, rtcXRoads } from './data/branches'
      ...
      <HashRouter future={{ v7_startTransition: true, v7_relativeSplatPath: true }}>
        ...
        <Suspense fallback={<PageLoader />}>
          <Routes>
            ...
            <Route path="/international-connentions" element={<InternationalPatientsPage />} />
            <Route path="/international-connections" element={<InternationalPatientsPage />} />
            <Route path="/international-patients" element={<Navigate to="/international-connentions" replace />} />
...
function FloatingAdminTrigger() {
  ...
  const handleReplayPreloader = () => {
    window.dispatchEvent(new Event('srikara_show_preloader'))
  }
  ...
  return (
    <div className="fixed bottom-4 left-4 z-[9999] flex items-center gap-2">
      <Link
        to="/admin"
        className="w-9 h-9 bg-white/95 hover:bg-[#8B1A4A] text-slate-400 hover:text-white rounded-full flex items-center justify-center shadow-lg border border-slate-200 backdrop-blur-sm transition-all duration-300 hover:scale-110 group"
      ...
      <button
        ...
        className="px-3 py-1.5 bg-slate-900/90 hover:bg-[#8B1A4A] text-white rounded-full flex items-center gap-1.5 text-[11px] font-bold shadow-lg border border-white/10 backdrop-blur-md transition-all duration-300 hover:scale-105 cursor-pointer"
        title="Replay Opening Announcement Sequence"
      >
