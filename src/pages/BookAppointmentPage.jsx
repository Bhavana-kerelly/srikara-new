...
export function BookAppointmentPage() {
  ...
  const handleFormSubmit = async (e) => {
    ...
    try {
      ...
      // Open WhatsApp with prefilled booking details
      const waNumber = selectedDoc?.whatsapp || '919247958308'
      const waMsg = encodeURIComponent(
        `Hello Srikara Hospitals,\n\nI have submitted an appointment request on the website. Here are my details:\n\n` +
        `👤 *Patient Name*: ${bookingName.trim()}\n` +
        `📱 *Mobile*: ${bookingPhone.trim()}\n` +
        `👨‍⚕️ *Consultant*: ${selectedDoc?.name || 'Any Consultant'}\n` +
        `🏥 *Branch*: ${selectedBranch?.title || 'Unknown Branch'}\n` +
        `📅 *Preferred Slot*: ${bookingDate} at ${bookingTime}\n\n` +
