...
function DoctorCard({ doctor, onView, onBook }) {
  ...
  const getDoctorScale = () => {
    if (doctor.imageScale) return doctor.imageScale
    const str = `${doctor.name || ''} ${doctor.slug || ''} ${doctor.image || ''}`.toLowerCase()
    if (str.includes('somesh') || str.includes('manjunath')) return 1.18
    if (str.includes('sravan')) return 1.18
    if (str.includes('siva') && (str.includes('gautham') || str.includes('gowtham'))) return 1.25
    if (str.includes('siva-gautham')) return 1.25
    if (str.includes('jyotheswara') || str.includes('jyotheshwar')) return 1.14
    if (str.includes('brahmananda')) return 1.14
    if (str.includes('meena-kumari') || str.includes('meenakumari')) return 1.14
    if (str.includes('padma')) return 1.14
    if (str.includes('santhosh') || str.includes('santosh')) return 1.14
    return 1.0
  }

  const scale = getDoctorScale()
  ...
  return (
    ...
    >
      {/* 1. PORTRAIT PHOTO CONTAINER WITH INTERACTIVE FADED BURGUNDY BACKGROUND */}
      <div 
        ref={cardRef}
        className="relative w-full h-[340px] rounded-[24px] overflow-hidden transition-all duration-500 bg-[#E8EDF2] group-hover:bg-gradient-to-b group-hover:from-[#F5E8EF] group-hover:to-[#E8B4C8] flex items-end justify-center pt-4"
        style={{ transformStyle: 'preserve-3d' }}
      >
        <div className="w-full h-full flex items-end justify-center overflow-hidden transition-transform duration-700 ease-out group-hover:scale-[1.04] origin-bottom">
          <img 
            src={doctor.image} 
            alt={doctor.name}
            className="w-auto max-w-[92%] h-[270px] max-h-[85%] object-contain object-bottom transition-all duration-500"
            style={{
...
export function DoctorsPage() {
  ...
  // Remove duplicates for the 'All' view and ensure Dr. Akhil Dadi comes first
  const uniqueFiltered = activeBranch === 'all'
    ? filtered.filter((doc, index, self) => index === self.findIndex(t => (t.slug || t.id) === (doc.slug || doc.id)))
    : filtered

  const displayDoctors = [...uniqueFiltered].sort((a, b) => {
    const isAkhilA = a.slug === 'dr-akhil-dadi' || (a.name && a.name.toLowerCase().includes('akhil dadi'))
    const isAkhilB = b.slug === 'dr-akhil-dadi' || (b.name && b.name.toLowerCase().includes('akhil dadi'))
    if (isAkhilA) return -1
    if (isAkhilB) return 1
    return 0
  })
            ...
            <div className="max-w-4xl mx-auto relative z-10 flex flex-col items-center">
              ...
              >
                ...
                <div className="flex flex-col gap-2.5 w-full items-center">
                  <div className="flex items-center gap-1.5 text-[#2D3A4A] shrink-0 font-display">
                    <MapPin size={14} className="text-[#8B1A4A]" />
                    <span className="text-xs font-black uppercase tracking-[0.2em]">Select City / Branch</span>
                  </div>
