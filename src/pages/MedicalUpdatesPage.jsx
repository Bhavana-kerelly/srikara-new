...
export function MedicalUpdatesPage() {
  ...
  const [allUpdates, setAllUpdates] = useState(DEFAULT_UPDATES)
    ...
    const loadNews = async () => {
      ...
      try {
        if (db) {
          ...
          if (!snap.empty) {
            liveNews = snap.docs.map(d => ({ id: d.id, ...d.data() }))
          }
        }
      } catch (err) {
        console.warn('Firestore news fetch error:', err)
      }
      ...
      if (liveNews.length === 0) {
        try {
          ...
          console.warn('Cache news fetch error:', e)
        }
      }

      if (liveNews.length > 0) {
        const validNews = liveNews.filter(n => n.status !== 'Deleted' && (n.title || n.headline))
        const sortedLiveNews = [...validNews].sort((a, b) => parseTime(b) - parseTime(a))

        const formatted = sortedLiveNews.map((n, i) => ({
          id: n.id || String(i),
          date: n.date || n.publishDate || 'Recent',
          category: n.type || n.category || 'Event',
          icon: Newspaper,
          title: n.title || n.headline,
          excerpt: n.desc || n.description || n.excerpt || n.content,
          accent: { bg: 'bg-[#FFF0F5]', text: 'text-[#8B1A4A]', border: 'border-[#FFE4E1]' },
          featured: i === 0
        }))

        const filteredDefault = DEFAULT_UPDATES.filter(du => 
          !formatted.some(fn => fn.title.toLowerCase() === du.title.toLowerCase())
        )

        // Keep ALL updates in the Medical Updates page archive
        setAllUpdates([...formatted, ...filteredDefault])
      }
    }

    loadNews()

    window.addEventListener('srikara_news_updated', loadNews)
    window.addEventListener('storage', loadNews)

    return () => {
      window.removeEventListener('srikara_news_updated', loadNews)
      window.removeEventListener('storage', loadNews)
    }
  }, [])

  const categories = ['All', ...new Set(allUpdates.map(u => u.category))]
  const updates = allUpdates.filter(u => category === 'All' || u.category === category)
